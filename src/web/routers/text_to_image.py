...
def to_image(text: str, image_mode: ImageMode) -> Image.Image:
    try:
        ...
        text_to_image_logger.warning('Could not create TextImage from text', exc_info=unicode_encode_error)
        raise HTTPException(
...
def get_image_file_bytes(image: Image.Image, image_format: str = 'png') -> BytesIO:
    buffer = BytesIO()
    try:
...
async def post__text_to_image(
    ...
    file_content = get_image_file_bytes(image, image_format=image_extension)
